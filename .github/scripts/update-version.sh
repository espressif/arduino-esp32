...
function version_bump_files {
    echo ".github/ISSUE_TEMPLATE/Issue-report.yml"
    echo ".gitlab/workflows/common.yml"
    echo "platform.txt"
    echo "package.json"
    echo "docs/conf_common.py"
    echo "cores/esp32/esp_arduino_version.h"
    find libraries -mindepth 2 -maxdepth 2 -name library.properties 2>/dev/null | sort
}

function note_bump_file {
    local path="$1"
    if [ -n "${VERSION_BUMP_FILES_LIST:-}" ]; then
        echo "$path" >> "$VERSION_BUMP_FILES_LIST"
    fi
}
...
if [ $# -ne 1 ]; then
    ...
    echo "usage: $0 [--print-files | <version>]" >&2
    exit 1
fi
...
if ! grep -q "^[[:space:]]*- v$version$" .github/ISSUE_TEMPLATE/Issue-report.yml; then
    ...
    note_bump_file ".github/ISSUE_TEMPLATE/Issue-report.yml"
...
note_bump_file "docs/conf_common.py"
...
note_bump_file "cores/esp32/esp_arduino_version.h"
...
for lib in $libraries; do
    if [ -f "libraries/$lib/library.properties" ]; then
        ...
        note_bump_file "libraries/$lib/library.properties"
    fi
done
