...
**Important:** ``onChange()`` is **not** automatically called when Matter commands are executed. Commands modify ``TargetPosition``, not ``CurrentPosition``. CHIP then sets ``OperationalStatus`` to Opening/Closing until the current lift position matches the target (Alexa keeps showing "Opening" and the old percent). Returning true from ``onGoToLiftPercentage()`` only accepts the target. The callback must update the current lift position (prefer ``setCurrentLiftPercent100ths(getTargetLiftPercent100ths())``) and call ``setOperationalState(LIFT, STALL)`` when the covering arrives. That also fires ``onChange()``.
