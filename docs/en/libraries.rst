...
.. [6] Matter-over-Thread is in the Arduino IDE prebuild for ESP32-C6 (dual-stack with Wi-Fi), ESP32-H2 (Thread only),
   and ESP32-C5 when **Tools → Matter Network → Thread** (``esp32c5_mot``). The default C5 menu (Wi-Fi) is Matter-over-Wi-Fi;
   OpenThread examples still link. Check ``Matter.isThreadEnabled()`` / ``CONFIG_ENABLE_MATTER_OVER_THREAD``.
