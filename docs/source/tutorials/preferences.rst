...
Before you can write or read anything using Preferences, both the namespace and the key within that namespace need to exist. So the workflow is:
...
.. note::
   ...
   From here on when referring in general to a method used to store or retrieve data we'll use the shorthand "``putX``" and "``getX``" where the "``X``" is understood to be a Preferences Type; Bool, UInt, Char, etc. from the "Preferences Data Types" table above.
...
If the namespace does not yet exist, this will create and and then open the namespace ``myPrefs``.
...
We do that by testing for the existence of a certain key within a namespace. If that key exists, then it safe to assume that that key was created during the first-time run of the sketch and so the namespace has been initialized already.
...
This returns ``true`` if  ``"myTestKey"`` already exists in the namespace, and ``false`` if it does not.
...
.. code-block:: arduino
   ...
   if (doesExist == false) {
       /* 
          If doesExist is false, we will need to create our namespace key(s) before
           we can put a value into them.
      */    
...
If we know we need to create a key, we use one of the  ``.putX`` methods, matching ``"X"`` to the type of data we wish to store:
...
Once a key exists in a namespace and the namespace is open, its value is retrieved using one of the ``getX`` methods, again matching ``"X"`` to the type of data we wish to retrieve.
...
The purpose of this part of the startup code is to use either a factory default configuration if the system has never run before, or the last configuration it had if not.
...
There are a number of other functions that are useful when working with namespaces.
...
For each namespace, Preferences keeps track of the keys in a key table. There must be an open entry in the table before a key can be created. This method will return how may entires are available in the key table.
...
will print to the serial monitor this information. 

the number of free entries in the key table changes depending on the number of keys in the namespace and also the dynamic size of certain types of data stored in the namespace. Details are in the `Preferences API document`_.
...
Keeping track of the data types stored against a key-value pair is one of the bookkeeping tasks left to the programmer. Should you want to discover the Preferences data type stored against a given key, use this method:
...
Recall that the Preferences library works best for storing many small values, rather than a few large values. However, there may be times when it is desirable to store larger amounts of arbitrary data than what is provided by the basic types in the Preferences Data Types table above.
...
This is best explained with an example. Here we use the ``Bytes`` methods to store and retrieve an array, while ensuring the data type is preserved.

.. code-block:: arduino

   /*
    *  An example sketch using the Preferences "Bytes" methods
    *   to store and retreive an arbitrary number of bytes in
    *   a namespace.
    */
...
You can copy the sketch and change the data type and values in ``myArray`` to and follow along with the code and output to see how the ``Bytes`` methods work. Remember to change the data type of ``myIntBuffer`` to match that of ``myArray``.
...
Assume we have a key named "``chocolate``" that can contain a value of a ``String`` data type.
...
* `Preferences API document`_ *What is the syntax for adding a link to the arduino-esp32 Preferences API document?*
* `Non-volatile storage library`_ (espressif-IDF API Reference)
* `Official ESP-IDF documentation`_ (espressif-IDF Reference)

.. _Preferences API document: https://preferences_library_api.html
