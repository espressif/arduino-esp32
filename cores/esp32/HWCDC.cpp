...
void HWCDC::begin(unsigned long baud)
{
    ...
    if(perimanGetPinBusType(pin) != ESP32_BUS_TYPE_INIT) if(!perimanClearPinBus(pin)) goto err;
    if(!perimanSetPinBus(pin, ESP32_BUS_TYPE_USB_DM, (void *) this, -1, -1)) goto err;
    pin = ESP32_BUS_TYPE_USB_DP;
    perimanSetBusDeinit(pin, HWCDC::deinit);
    if(perimanGetPinBusType(pin) != ESP32_BUS_TYPE_INIT && !perimanClearPinBus(pin)) goto err;
...
void HWCDC::end()
{
    ...
    usb_serial_jtag_ll_disable_intr_mask(USB_SERIAL_JTAG_LL_INTR_MASK);  // disable all
    usb_serial_jtag_ll_clr_intsts_mask(USB_SERIAL_JTAG_LL_INTR_MASK); // clear pending
