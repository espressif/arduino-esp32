...
def test_ble(dut, ci_job_id):
    ...
    m = server.expect(r"\[SERVER\] Service UUID: ([0-9a-f\-]+)", timeout=10)
    service_uuid = m.group(1).decode()
    assert service_uuid == "4fafc201-1fb5-459e-8fcc-c5c9c331914b"
    LOGGER.info(f"Server Service UUID: {service_uuid}")

    # Wait for client to start scanning
    LOGGER.info("Waiting for client to start scanning...")
    client.expect(rf"\[CLIENT\] Scanning for server: {re.escape(server_name)}", timeout=10)
