...
| `test_update_sha512_format` | `setSHA512` accepts a 128-character hex digest and rejects non-hex input |
| `test_update_sha512_roundtrip` | Local round trip for SHA-512: correct/wrong digests, MD5+SHA-256+SHA-512 combined |
...
| `test_httpupdate_invalid_checksums_abort` | Invalid MD5/SHA-256/SHA-512 values abort the update session |
| `test_httpupdate_wrong_sha256_has_no_digest` | A SHA-256 mismatch fails without exposing a digest |
| `test_httpupdate_wrong_sha512_has_no_digest` | A SHA-512 mismatch fails without exposing a digest |
...
| `test_httpupdate_sidecar_sha512_missing` | Missing SHA-512 sidecar URL fails with `HTTP_UE_SERVER_FAULTY_SHA512` |
| `test_httpupdate_sidecar_sha512_invalid` | SHA-512 sidecar without a valid hex token fails |
| `test_httpupdate_sidecar_wrong_sha512` | SHA-512 sidecar with wrong digest fails the firmware verify |
...
| `test_httpupdate_header_overrides_sidecar_sha512` | Correct `x-SHA512` header wins over a wrong sidecar URL |
| `test_httpupdate_setter_overrides_sidecar` | Explicit `setSHA256sum()` wins over a wrong sidecar URL |
| `test_httpupdate_setter_overrides_sidecar_sha512` | Explicit `setSHA512sum()` wins over a wrong sidecar URL |
| `test_httpupdate_sidecar_sha256` | SHA-256 sidecar (GNU `sha256sum` format) verifies firmware without header |
| `test_httpupdate_sidecar_sha512` | SHA-512 sidecar (GNU `sha512sum` format) verifies firmware without header |
...
| `test_httpupdate_both_sidecars` | MD5, SHA-256 and SHA-512 sidecars are fetched and verified together |
...
| `test_httpupdate_download` | `HTTPUpdate` download and SHA-256/SHA-512 verification over IPv4 |
...
- The pytest HTTP server serves the firmware with its SHA-256 and SHA-512 digests in `x-SHA256` / `x-SHA512` response headers for `/ota.ino.bin`, prefers dual-stack (`::` with `IPV6_V6ONLY=0`), and falls back to IPv4-only if IPv6 bind fails.
