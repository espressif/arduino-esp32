...
def test_wifi_ap(dut, ci_job_id):
    ...
    client.expect(rf"\[CLIENT\] Connecting to SSID={re.escape(ap_ssid)} Password={re.escape(ap_password)}", timeout=20)
    ...
    ap.expect(r"\[AP\] Stations connected: 1", timeout=30)

    # Verify client stays connected
    LOGGER.info(f"Waiting for client to report connected status")
    client.expect(r"\[CLIENT\] Status=3", timeout=10)  # WL_CONNECTED
