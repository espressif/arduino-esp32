...
Preferences::Preferences()
    : _handle(0), _started(false), _readOnly(false), _lastError(ESP_OK)
{
}
...
bool Preferences::begin(const char *name, bool readOnly)
{
    ...
    _readOnly = readOnly;
    _lastError = nvs_open(name, readOnly ? NVS_READONLY : NVS_READWRITE, &_handle);
    if (_lastError)
    {
        log_e("nvs_open failed: %s", nvs_error(_lastError));
        return false;
    }
    _started = true;
    return true;
...
bool Preferences::clear()
{
    ...
    _lastError = nvs_erase_all(_handle);
    if (_lastError)
    {
        log_e("nvs_erase_all fail: %s", nvs_error(_lastError));
        return false;
    }
    return true;
}
...
bool Preferences::remove(const char *key)
{
    ...
    _lastError = nvs_erase_key(_handle, key);
    if (_lastError)
    {
        log_e("nvs_erase_key fail: %s %s", key, nvs_error(_lastError));
        return false;
    }
    return true;
}
...
size_t Preferences::putChar(const char *key, int8_t value)
{
    ...
    _lastError = nvs_set_i8(_handle, key, value);
    if (_lastError)
    {
        log_e("nvs_set_i8 fail: %s %s", key, nvs_error(_lastError));
        return 0;
    }
    _lastError = nvs_commit(_handle);
    if (_lastError)
    {
        log_e("nvs_commit fail: %s %s", key, nvs_error(_lastError));
        return 0;
    }
    return 1;
}
...
size_t Preferences::putUChar(const char *key, uint8_t value)
{
    ...
    _lastError = nvs_set_u8(_handle, key, value);
    if (_lastError)
    {
        log_e("nvs_set_u8 fail: %s %s", key, nvs_error(_lastError));
        return 0;
    }
    _lastError = nvs_commit(_handle);
    if (_lastError)
...
size_t Preferences::putShort(const char *key, int16_t value)
{
    ...
    _lastError = nvs_set_i16(_handle, key, value);
    if (_lastError)
    {
        log_e("nvs_set_i16 fail: %s %s", key, nvs_error(_lastError));
        return 0;
    }
    _lastError = nvs_commit(_handle);
    if (_lastError)
...
size_t Preferences::putUShort(const char *key, uint16_t value)
{
    ...
    _lastError = nvs_set_u16(_handle, key, value);
    if (_lastError)
    {
        log_e("nvs_set_u16 fail: %s %s", key, nvs_error(_lastError));
        return 0;
    }
    _lastError = nvs_commit(_handle);
    if (_lastError)
...
size_t Preferences::putInt(const char *key, int32_t value)
{
    ...
    _lastError = nvs_set_i32(_handle, key, value);
    if (_lastError)
    {
        log_e("nvs_set_i32 fail: %s %s", key, nvs_error(_lastError));
        return 0;
    }
    _lastError = nvs_commit(_handle);
    if (_lastError)
...
size_t Preferences::putUInt(const char *key, uint32_t value)
{
    ...
    _lastError = nvs_set_u32(_handle, key, value);
    if (_lastError)
    {
        log_e("nvs_set_u32 fail: %s %s", key, nvs_error(_lastError));
        return 0;
    }
    _lastError = nvs_commit(_handle);
    if (_lastError)
...
size_t Preferences::putLong64(const char *key, int64_t value)
{
    ...
    _lastError = nvs_set_i64(_handle, key, value);
    if (_lastError)
    {
        log_e("nvs_set_i64 fail: %s %s", key, nvs_error(_lastError));
        return 0;
    }
    _lastError = nvs_commit(_handle);
    if (_lastError)
...
size_t Preferences::putULong64(const char *key, uint64_t value)
{
    ...
    _lastError = nvs_set_u64(_handle, key, value);
    if (_lastError)
    {
        log_e("nvs_set_u64 fail: %s %s", key, nvs_error(_lastError));
        return 0;
    }
    _lastError = nvs_commit(_handle);
    if (_lastError)
...
size_t Preferences::putString(const char *key, const char *value)
{
    ...
    _lastError = nvs_set_str(_handle, key, value);
    if (_lastError)
    {
        log_e("nvs_set_str fail: %s %s", key, nvs_error(_lastError));
        return 0;
    }
    _lastError = nvs_commit(_handle);
    if (_lastError)
...
size_t Preferences::putBytes(const char *key, const void *value, size_t len)
{
    ...
    _lastError = nvs_set_blob(_handle, key, value, len);
    if (_lastError)
    {
        log_e("nvs_set_blob fail: %s %s", key, nvs_error(_lastError));
        return 0;
    }
    _lastError = nvs_commit(_handle);
    if (_lastError)
...
int8_t Preferences::getChar(const char *key, const int8_t defaultValue)
{
    ...
    _lastError = nvs_get_i8(_handle, key, &value);
    if (_lastError)
    {
        log_v("nvs_get_i8 fail: %s %s", key, nvs_error(_lastError));
    }
    return value;
}
...
uint8_t Preferences::getUChar(const char *key, const uint8_t defaultValue)
{
    ...
    _lastError = nvs_get_u8(_handle, key, &value);
    if (_lastError)
    {
        log_v("nvs_get_u8 fail: %s %s", key, nvs_error(_lastError));
    }
    return value;
}
...
int16_t Preferences::getShort(const char *key, const int16_t defaultValue)
{
    ...
    _lastError = nvs_get_i16(_handle, key, &value);
    if (_lastError)
    {
        log_v("nvs_get_i16 fail: %s %s", key, nvs_error(_lastError));
    }
    return value;
}
...
uint16_t Preferences::getUShort(const char *key, const uint16_t defaultValue)
{
    ...
    _lastError = nvs_get_u16(_handle, key, &value);
    if (_lastError)
    {
        log_v("nvs_get_u16 fail: %s %s", key, nvs_error(_lastError));
    }
    return value;
}
...
int32_t Preferences::getInt(const char *key, const int32_t defaultValue)
{
    ...
    _lastError = nvs_get_i32(_handle, key, &value);
    if (_lastError)
    {
        log_v("nvs_get_i32 fail: %s %s", key, nvs_error(_lastError));
    }
    return value;
}
...
uint32_t Preferences::getUInt(const char *key, const uint32_t defaultValue)
{
    ...
    _lastError = nvs_get_u32(_handle, key, &value);
    if (_lastError)
    {
        log_v("nvs_get_u32 fail: %s %s", key, nvs_error(_lastError));
    }
    return value;
}
...
int64_t Preferences::getLong64(const char *key, const int64_t defaultValue)
{
    ...
    _lastError = nvs_get_i64(_handle, key, &value);
    if (_lastError)
    {
        log_v("nvs_get_i64 fail: %s %s", key, nvs_error(_lastError));
    }
    return value;
}
...
uint64_t Preferences::getULong64(const char *key, const uint64_t defaultValue)
{
    ...
    _lastError = nvs_get_u64(_handle, key, &value);
    if (_lastError)
    {
        log_v("nvs_get_u64 fail: %s %s", key, nvs_error(_lastError));
    }
    return value;
}
...
size_t Preferences::getString(const char *key, char *value, const size_t maxLen)
{
    ...
    _lastError = nvs_get_str(_handle, key, NULL, &len);
    if (_lastError)
    {
        log_e("nvs_get_str len fail: %s %s", key, nvs_error(_lastError));
        return 0;
    }
    if (len > maxLen)
    {
    ...
    _lastError = nvs_get_str(_handle, key, value, &len);
    if (_lastError)
    {
        log_e("nvs_get_str fail: %s %s", key, nvs_error(_lastError));
        return 0;
    }
    return len;
}
...
size_t Preferences::getBytesLength(const char *key)
{
    ...
    _lastError = nvs_get_blob(_handle, key, NULL, &len);
    if (_lastError)
    {
        log_e("nvs_get_blob len fail: %s %s", key, nvs_error(_lastError));
        return 0;
    }
    return len;
}
...
size_t Preferences::getBytes(const char *key, void *buf, size_t maxLen)
{
    ...
    _lastError = nvs_get_blob(_handle, key, buf, &len);
    if (_lastError)
    {
        log_e("nvs_get_blob fail: %s %s", key, nvs_error(_lastError));
        return 0;
    }
    return len;
}
...
size_t Preferences::freeEntries()
{
    ...
    _lastError = nvs_get_stats(NULL, &nvs_stats);
    if (_lastError)
    {
