...
/* Keyboard LED page, as sent to the keyboard in the OUTPUT report. */
#define USBHOST_KEY_LED_NUM_LOCK    0x01
#define USBHOST_KEY_LED_CAPS_LOCK   0x02
#define USBHOST_KEY_LED_SCROLL_LOCK 0x04
#define USBHOST_KEY_LED_COMPOSE     0x08
#define USBHOST_KEY_LED_KANA        0x10
 ...
 *
 * @p leds is the lock state (USBHOST_KEY_LED_*): Caps Lock inverts Shift for letters, and the
 * keypad digits only decode with Num Lock on. The layout table does not cover the keypad.
 */
char usbHostHidBootReportUsageToAscii(uint8_t modifiers, uint8_t hid_usage, uint8_t leds, const uint8_t *layout);
 ...
 * @p leds same as usbHostHidBootReportUsageToAscii().
