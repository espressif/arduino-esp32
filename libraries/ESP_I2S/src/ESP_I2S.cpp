...
bool I2SClass::initSTD(uint32_t rate, i2s_data_bit_width_t bits_cfg, i2s_slot_mode_t ch){
    ...
    if (_mclk >= 0) if (!perimanDetachPin(_mclk)){ return false; }
    if (_bclk >= 0) if (!perimanDetachPin(_bclk)){ return false; }
    if (_ws >= 0)   if (!perimanDetachPin(_ws))  { return false; }
    if (_dout >= 0) if (!perimanDetachPin(_dout)){ return false; }
    if (_din >= 0)  if (!perimanDetachPin(_din)) { return false; }
    ...
    if (_mclk >= 0) if (!perimanSetPinBus(_mclk, ESP32_BUS_TYPE_I2S_STD_MCLK, (void *)(this))){ goto err; }
    if (_bclk >= 0) if (!perimanSetPinBus(_bclk, ESP32_BUS_TYPE_I2S_STD_BCLK, (void *)(this))){ goto err; }
    if (_ws >= 0)   if (!perimanSetPinBus(_ws,   ESP32_BUS_TYPE_I2S_STD_WS,   (void *)(this))){ goto err; }
    if (_dout >= 0) if (!perimanSetPinBus(_dout, ESP32_BUS_TYPE_I2S_STD_DOUT, (void *)(this))){ goto err; }
    if (_din >= 0)  if (!perimanSetPinBus(_din,  ESP32_BUS_TYPE_I2S_STD_DIN,  (void *)(this))){ goto err; }
...
bool I2SClass::initTDM(uint32_t rate, i2s_data_bit_width_t bits_cfg, i2s_slot_mode_t ch, int8_t slot_mask){
    ...
    // Peripheral manager set bus type to I2S
    if (_mclk >= 0) if (!perimanSetPinBus(_mclk, ESP32_BUS_TYPE_I2S_TDM_MCLK, (void *)(this))){ goto err; }
    if (_bclk >= 0) if (!perimanSetPinBus(_bclk, ESP32_BUS_TYPE_I2S_TDM_BCLK, (void *)(this))){ goto err; }
    if (_ws >= 0)   if (!perimanSetPinBus(_ws,   ESP32_BUS_TYPE_I2S_TDM_WS,   (void *)(this))){ goto err; }
    if (_dout >= 0) if (!perimanSetPinBus(_dout, ESP32_BUS_TYPE_I2S_TDM_DOUT, (void *)(this))){ goto err; }
    if (_din >= 0)  if (!perimanSetPinBus(_din,  ESP32_BUS_TYPE_I2S_TDM_DIN,  (void *)(this))){ goto err; }
...
bool I2SClass::initPDMtx(uint32_t rate, i2s_data_bit_width_t bits_cfg, i2s_slot_mode_t ch){
    // Peripheral manager deinit previous peripheral if pin was used
    if (_tx_clk >= 0)   if (!perimanDetachPin(_tx_clk))  { return false; }
    if (_tx_dout0 >= 0) if (!perimanDetachPin(_tx_dout0)){ return false; }
    if (_tx_dout1 >= 0) if (!perimanDetachPin(_tx_dout1)){ return false; }
    ...
    // Peripheral manager set bus type to I2S
    if (_tx_clk >= 0)   if (!perimanSetPinBus(_tx_clk,   ESP32_BUS_TYPE_I2S_PDM_TX_CLK,   (void *)(this))){ goto err; }
    if (_tx_dout0 >= 0) if (!perimanSetPinBus(_tx_dout0, ESP32_BUS_TYPE_I2S_PDM_TX_DOUT0, (void *)(this))){ goto err; }
    if (_tx_dout1 >= 0) if (!perimanSetPinBus(_tx_dout1, ESP32_BUS_TYPE_I2S_PDM_TX_DOUT1, (void *)(this))){ goto err; }
...
bool I2SClass::initPDMrx(uint32_t rate, i2s_data_bit_width_t bits_cfg, i2s_slot_mode_t ch){
    // Peripheral manager deinit previous peripheral if pin was used
    if (_rx_clk >= 0)  if (!perimanDetachPin(_rx_clk)) { return false; }
    if (_rx_din0 >= 0) if (!perimanDetachPin(_rx_din0)){ return false; }
    if (_rx_din1 >= 0) if (!perimanDetachPin(_rx_din1)){ return false; }
    if (_rx_din2 >= 0) if (!perimanDetachPin(_rx_din2)){ return false; }
    if (_rx_din3 >= 0) if (!perimanDetachPin(_rx_din3)){ return false; }
    ...
    // Peripheral manager set bus type to I2S
    if (_rx_clk >= 0)  if (!perimanSetPinBus(_rx_clk,  ESP32_BUS_TYPE_I2S_PDM_RX_CLK,  (void *)(this))){ goto err; }
    if (_rx_din0 >= 0) if (!perimanSetPinBus(_rx_din0, ESP32_BUS_TYPE_I2S_PDM_RX_DIN0, (void *)(this))){ goto err; }
    if (_rx_din1 >= 0) if (!perimanSetPinBus(_rx_din1, ESP32_BUS_TYPE_I2S_PDM_RX_DIN1, (void *)(this))){ goto err; }
    if (_rx_din2 >= 0) if (!perimanSetPinBus(_rx_din2, ESP32_BUS_TYPE_I2S_PDM_RX_DIN2, (void *)(this))){ goto err; }
    if (_rx_din3 >= 0) if (!perimanSetPinBus(_rx_din3, ESP32_BUS_TYPE_I2S_PDM_RX_DIN3, (void *)(this))){ goto err; }
...
bool I2SClass::end(){
    ...
    switch (_mode){
        ...
        case I2S_MODE_TDM:
#endif
            perimanDetachPin(_mclk);
            perimanDetachPin(_bclk);
            perimanDetachPin(_ws);
            if (_dout >= 0) perimanDetachPin(_dout);
            if (_din >= 0)  perimanDetachPin(_din);
            break;
#if SOC_I2S_SUPPORTS_PDM_TX
        case I2S_MODE_PDM_TX:
            perimanDetachPin(_tx_clk);
            if (_tx_dout0 >= 0) perimanDetachPin(_tx_dout0);
            if (_tx_dout1 >= 0) perimanDetachPin(_tx_dout1);
            break;
#endif
#if SOC_I2S_SUPPORTS_PDM_RX
        case I2S_MODE_PDM_RX:
            perimanDetachPin(_rx_clk);
            if (_rx_din0 >= 0) perimanDetachPin(_rx_din0);
            if (_rx_din1 >= 0) perimanDetachPin(_rx_din1);
            if (_rx_din2 >= 0) perimanDetachPin(_rx_din2);
            if (_rx_din3 >= 0) perimanDetachPin(_rx_din3);
            break;
#endif
        default:
            break;
