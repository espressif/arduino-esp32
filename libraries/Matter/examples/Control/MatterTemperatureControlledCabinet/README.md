...
**Important:** The `temperature_number` and `temperature_level` features are **mutually exclusive**. Only one can be enabled at a time. See the [MatterTemperatureControlledCabinetLevels](../MatterTemperatureControlledCabinetLevels) example for temperature level control mode.
...
Use this mode when you need precise temperature control with specific setpoint values (e.g., 4.0°C for a refrigerator, -18.0°C for a freezer). For preset-based temperature control using levels, see the [MatterTemperatureControlledCabinetLevels](../MatterTemperatureControlledCabinetLevels) example.
