...
1. **`setup()`**: Initializes hardware (button, LED), configures Wi-Fi (if needed), sets up the Matter Color Temperature Light endpoint, restores the last known state from `Preferences` (on/off, brightness, color temperature), registers callbacks for state changes, and waits for commissioning / CASE via `matterWaitUntilReady()`.
