...
This table is what **this sketch** does on the Arduino IDE prebuild. It does not call `Matter.selectNetwork()` or start Ethernet.

CHIPoBLE is compiled in only when `CONFIG_ENABLE_CHIPOBLE` is set:

- **Arduino IDE:** original ESP32 is Bluedroid (no CHIPoBLE) and ESP32-S2 has no Bluetooth. Those two call `matterConnectWiFi(WIFI_SSID, WIFI_PASSWORD)`. C3/C5/C6/S3/H2 use NimBLE + CHIPoBLE.
...
- [Matter Smart Buttons TagList](../../Control/MatterSmartButtonsTagList) — Descriptor tags (not light names)
