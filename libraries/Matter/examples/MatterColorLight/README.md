...
Matter 1.5 has no Color Light device type. This example uses ``MatterColorLight``, which is advertised as an Extended Color Light with Hue/Saturation and XY **only**. Color Temperature is not in the data model. For hue/saturation, XY, and color temperature, use the [Matter Enhanced Color Light](https://github.com/espressif/arduino-esp32/tree/master/libraries/Matter/examples/MatterEnhancedColorLight) example.
