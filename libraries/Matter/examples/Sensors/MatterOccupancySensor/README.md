...
1. **`setup()`**: Initializes hardware (button), configures Wi-Fi (if needed), sets up the Matter Occupancy Sensor endpoint with initial state (unoccupied), and waits for commissioning / CASE via `matterWaitUntilReady()`.

2. **`loop()`**: Continuously checks the simulated occupancy sensor and updates the Matter attribute, and handles long-press decommission. `matterRestartIfNoFabric()` reboots if the hub removed the fabric.
