...
1. **`setup()`**: Initializes hardware (button), configures Wi-Fi (if needed), sets up the Matter Temperature Sensor endpoint with initial value (-25°C), and waits for commissioning / CASE via `matterWaitUntilReady()`.

2. **`loop()`**: Displays the current temperature value every 5 seconds, updates the sensor reading from the simulated hardware sensor, and handles long-press decommission. `matterRestartIfNoFabric()` reboots if the hub removed the fabric.
