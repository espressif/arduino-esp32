  ...
  if (endpoint_id == getEndPointId() && cluster_id == FanControl::Id) {
    ...
    switch (attribute_id) {
        ...
        case  FanControl::Attributes::PercentCurrent::Id:
            log_d("FanControl Percent %s changed to %d", attribute_id ==  FanControl::Attributes::PercentSetting::Id ? "SETTING" : "CURRENT", val->val.u8);
...
bool MatterFan::setMode(FanMode_t newMode, bool performUpdate) {
  ...
  log_d("Fan Mode %s to %s ==> onOffState[%s]", performUpdate ? "updated" : "set", fanModeString[currentFanMode], getOnOff() ? "ON" : "OFF");
  return true;
...
bool MatterFan::setSpeedPercent(uint8_t newPercent, bool performUpdate) {
  ...
  log_d("Fan Speed %s to %d ==> onOffState[%s]", performUpdate ? "updated" : "set", currentPercent, getOnOff() ? "ON" : "OFF");
  return true;  
}
...
bool MatterFan::setOnOff(bool newState, bool performUpdate) {
  ...
  log_d("Fan State %s to %s :: Mode[%s]|Speed[%d]", performUpdate ? "updated" : "set", getOnOff() ? "ON" : "OFF", fanModeString[currentFanMode], currentPercent);
  return true;
}
