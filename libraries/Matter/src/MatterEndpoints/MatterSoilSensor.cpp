...
void ESPMatterSoilMeasurementClusterServerInitCallback(chip::EndpointId endpoint_id) {
  ...
  if (err != CHIP_NO_ERROR) {
    ...
    // Registration failed: undo the construction so IsConstructed() doesn't short-circuit a future retry.
    gSoilServers[endpoint_id].Destroy();
    gSoilServers.erase(endpoint_id);
  }
}
