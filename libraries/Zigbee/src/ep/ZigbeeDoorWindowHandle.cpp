...
ezb_af_ep_desc_t zigbee_door_window_handle_create_ep_desc(uint8_t endpoint, zigbee_door_window_handle_cfg_t *door_window_handle) {
  ...
  const void *basic_cfg = door_window_handle ? &(door_window_handle->basic_cfg) : nullptr;
  const void *identify_cfg = door_window_handle ? &(door_window_handle->identify_cfg) : nullptr;
  const void *ias_zone_cfg = door_window_handle ? &(door_window_handle->ias_zone_cfg) : nullptr;
...
ZigbeeDoorWindowHandle::ZigbeeDoorWindowHandle(uint8_t endpoint) : ZigbeeEP(endpoint) {
  ...
  _ep_desc = zigbee_door_window_handle_create_ep_desc(_endpoint, &door_window_handle_cfg);
