...
ZigbeeRangeExtender::ZigbeeRangeExtender(uint8_t endpoint) : ZigbeeEP(endpoint) {
  ...
  // Endpoint descriptor: Basic + Identify. Registered as a Range Extender device.
  ezb_af_ep_config_t ep_config = {
    .ep_id = _endpoint, .app_profile_id = EZB_AF_HA_PROFILE_ID, .app_device_id = EZB_ZHA_RANGE_EXTENDER_DEVICE_ID, .app_device_version = 0, .reserved = 0
  };
