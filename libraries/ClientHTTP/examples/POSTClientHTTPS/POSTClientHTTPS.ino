...
void setup() {
  ...
  if(!http.POST("/post", sizeof payload - 1)) { // The terminating '\0' should not be send
    Serial.println("Failed to POST");
    return;
  }

  http.write((const uint8_t *)payload, sizeof payload - 1);  // The terminating '\0' should not be send
