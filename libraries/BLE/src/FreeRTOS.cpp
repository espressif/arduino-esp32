...
uint32_t FreeRTOS::Semaphore::wait(std::string owner) {
	ESP_LOGV(">> wait: Semaphore waiting: %s for %s", toString().c_str(), owner.c_str());
	...
	ESP_LOGV("<< wait: Semaphore released: %s", toString().c_str());
...
void FreeRTOS::Semaphore::give() {
	ESP_LOGV("Semaphore giving: %s", toString().c_str());
	if (m_usePthreads) {
		pthread_mutex_unlock(&m_pthread_mutex);
	} else {
		xSemaphoreGive(m_semaphore);
...
bool FreeRTOS::Semaphore::take(std::string owner) {
	ESP_LOGD("Semaphore taking: %s for %s", toString().c_str(), owner.c_str());
	bool rc = false;
	if (m_usePthreads) {
		pthread_mutex_lock(&m_pthread_mutex);
	} else {
	...
	m_owner = owner;
	if (rc) {
		ESP_LOGD("Semaphore taken:  %s", toString().c_str());
	} else {
		ESP_LOGE("Semaphore NOT taken:  %s", toString().c_str());
	}
	return rc;
} // Semaphore::take
...
bool FreeRTOS::Semaphore::take(uint32_t timeoutMs, std::string owner) {
	ESP_LOGV("Semaphore taking: %s for %s", toString().c_str(), owner.c_str());
	bool rc = false;
	if (m_usePthreads) {
	...
	m_owner = owner;
	if (rc) {
		ESP_LOGV("Semaphore taken:  %s", toString().c_str());
	} else {
		ESP_LOGE("Semaphore NOT taken:  %s", toString().c_str());
	}
	return rc;
