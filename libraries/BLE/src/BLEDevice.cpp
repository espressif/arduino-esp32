...
/* STATIC */ BLEClient* BLEDevice::createClient() {
	ESP_LOGV(">> createClient");
#ifndef CONFIG_GATTC_ENABLE  // Check that BLE GATTC is enabled in make menuconfig
	ESP_LOGE("BLE GATTC is not enabled - CONFIG_GATTC_ENABLE not defined");
	abort();
...
/* STATIC */ BLEServer* BLEDevice::createServer() {
	ESP_LOGV(">> createServer");
#ifndef CONFIG_GATTS_ENABLE  // Check that BLE GATTS is enabled in make menuconfig
	ESP_LOGE("BLE GATTS is not enabled - CONFIG_GATTS_ENABLE not defined");
	abort();
...
/* STATIC */ void BLEDevice::gattServerEventHandler(
   ...
   esp_ble_gatts_cb_param_t* param
	...
	ESP_LOGD("gattServerEventHandler [esp_gatt_if: %d] ... %s",
		gatts_if,
...
/* STATIC */ void BLEDevice::gattClientEventHandler(
	...
	ESP_LOGD("gattClientEventHandler [esp_gatt_if: %d] ... %s",
...
/* STATIC */ void BLEDevice::gapEventHandler(
	...
	switch(event) {
		 ...
		 case ESP_GAP_BLE_OOB_REQ_EVT:                                /* OOB request event */
			 ESP_LOGI("ESP_GAP_BLE_OOB_REQ_EVT");
			 break;
		 case ESP_GAP_BLE_LOCAL_IR_EVT:                               /* BLE local IR event */
			 ESP_LOGI("ESP_GAP_BLE_LOCAL_IR_EVT");
			 break;
		 case ESP_GAP_BLE_LOCAL_ER_EVT:                               /* BLE local ER event */
			 ESP_LOGI("ESP_GAP_BLE_LOCAL_ER_EVT");
			 break;
		 case ESP_GAP_BLE_NC_REQ_EVT:								/*  NUMERIC CONFIRMATION  */
			 ESP_LOGI("ESP_GAP_BLE_NC_REQ_EVT");
#ifdef CONFIG_BLE_SMP_ENABLE   // Check that BLE SMP (security) is configured in make menuconfig
		 ...
		 case ESP_GAP_BLE_PASSKEY_REQ_EVT:                           /* passkey request event */
			ESP_LOGI("ESP_GAP_BLE_PASSKEY_REQ_EVT: ");
		 ...
		 case ESP_GAP_BLE_SEC_REQ_EVT:
			 ...
			 ESP_LOGI("ESP_GAP_BLE_SEC_REQ_EVT");
#ifdef CONFIG_BLE_SMP_ENABLE   // Check that BLE SMP (security) is configured in make menuconfig
			if(BLEDevice::m_securityCallbacks!=nullptr){
		 ...
		 case ESP_GAP_BLE_PASSKEY_NOTIF_EVT:  //the app will receive this evt when the IO  has Output capability and the peer device IO has Input capability.
			 ...
			 ESP_LOGI("ESP_GAP_BLE_PASSKEY_NOTIF_EVT");
#ifdef CONFIG_BLE_SMP_ENABLE   // Check that BLE SMP (security) is configured in make menuconfig
			ESP_LOGI("passKey = %d", param->ble_security.key_notif.passkey);
			if(BLEDevice::m_securityCallbacks!=nullptr){
		 ...
		 case ESP_GAP_BLE_KEY_EVT:
			 ...
			 ESP_LOGD("ESP_GAP_BLE_KEY_EVT");
#ifdef CONFIG_BLE_SMP_ENABLE   // Check that BLE SMP (security) is configured in make menuconfig
			 ESP_LOGI("key type = %s", BLESecurity::esp_key_type_to_str(param->ble_security.ble_key.key_type));
#endif	// CONFIG_BLE_SMP_ENABLE
			 break;
		 case ESP_GAP_BLE_AUTH_CMPL_EVT:
			 ESP_LOGI("ESP_GAP_BLE_AUTH_CMPL_EVT");
#ifdef CONFIG_BLE_SMP_ENABLE   // Check that BLE SMP (security) is configured in make menuconfig
			 if(BLEDevice::m_securityCallbacks != nullptr){
...
/* STATIC */ BLEScan* BLEDevice::getScan() {
	...
	if (m_pScan == nullptr) {
		m_pScan = new BLEScan();
		//ESP_LOGD(" - creating a new scan object");
	}
	//ESP_LOGV("<< getScan: Returning object at 0x%x", (uint32_t)m_pScan);
	return m_pScan;
} // getScan
...
/* STATIC */ std::string BLEDevice::getValue(BLEAddress bdAddress, BLEUUID serviceUUID, BLEUUID characteristicUUID) {
	ESP_LOGV(">> getValue: bdAddress: %s, serviceUUID: %s, characteristicUUID: %s", bdAddress.toString().c_str(), serviceUUID.toString().c_str(), characteristicUUID.toString().c_str());
...
/* STATIC */ void BLEDevice::init(std::string deviceName) {
	if(!initialized){
		...
		if (errRc != ESP_OK) {
			ESP_LOGE("nvs_flash_init: rc=%d %s", errRc, GeneralUtils::errorToString(errRc));
			return;
		}
		...
		if (errRc != ESP_OK) {
			ESP_LOGE("esp_bt_controller_init: rc=%d %s", errRc, GeneralUtils::errorToString(errRc));
			return;
		}
		...
		if (errRc != ESP_OK) {
			ESP_LOGE("esp_bt_controller_enable: rc=%d %s", errRc, GeneralUtils::errorToString(errRc));
			return;
		}
#else
		...
		if (bt_state == ESP_BLUEDROID_STATUS_UNINITIALIZED) {
			...
			if (errRc != ESP_OK) {
				ESP_LOGE("esp_bluedroid_init: rc=%d %s", errRc, GeneralUtils::errorToString(errRc));
				return;
			}
		}
		...
		if (bt_state != ESP_BLUEDROID_STATUS_ENABLED) {
			...
			if (errRc != ESP_OK) {
				ESP_LOGE("esp_bluedroid_enable: rc=%d %s", errRc, GeneralUtils::errorToString(errRc));
				return;
			}
		}
		...
		if (errRc != ESP_OK) {
			ESP_LOGE("esp_ble_gap_register_callback: rc=%d %s", errRc, GeneralUtils::errorToString(errRc));
			return;
		}
		...
		if (errRc != ESP_OK) {
			ESP_LOGE("esp_ble_gattc_register_callback: rc=%d %s", errRc, GeneralUtils::errorToString(errRc));
			return;
		}
#endif   // CONFIG_GATTC_ENABLE
		...
		if (errRc != ESP_OK) {
			ESP_LOGE("esp_ble_gatts_register_callback: rc=%d %s", errRc, GeneralUtils::errorToString(errRc));
			return;
		}
#endif   // CONFIG_GATTS_ENABLE
		...
		if (errRc != ESP_OK) {
			ESP_LOGE("esp_ble_gap_set_device_name: rc=%d %s", errRc, GeneralUtils::errorToString(errRc));
			return;
		};
		...
		if (errRc != ESP_OK) {
			ESP_LOGE("esp_ble_gap_set_security_param: rc=%d %s", errRc, GeneralUtils::errorToString(errRc));
			return;
		};
#endif // CONFIG_BLE_SMP_ENABLE
	}
...
/* STATIC */ void BLEDevice::setPower(esp_power_level_t powerLevel) {
	ESP_LOGV(">> setPower: %d", powerLevel);
	esp_err_t errRc = ::esp_ble_tx_power_set(ESP_BLE_PWR_TYPE_DEFAULT, powerLevel);
	if (errRc != ESP_OK) {
		ESP_LOGE("esp_ble_tx_power_set: rc=%d %s", errRc, GeneralUtils::errorToString(errRc));
	};
	ESP_LOGV("<< setPower");
} // setPower
...
/* STATIC */ void BLEDevice::setValue(BLEAddress bdAddress, BLEUUID serviceUUID, BLEUUID characteristicUUID, std::string value) {
	ESP_LOGV(">> setValue: bdAddress: %s, serviceUUID: %s, characteristicUUID: %s", bdAddress.toString().c_str(), serviceUUID.toString().c_str(), characteristicUUID.toString().c_str());
	BLEClient* pClient = createClient();
	pClient->connect(bdAddress);
...
void BLEDevice::whiteListAdd(BLEAddress address) {
	ESP_LOGV(">> whiteListAdd: %s", address.toString().c_str());
	esp_err_t errRc = esp_ble_gap_update_whitelist(true, *address.getNative());  // True to add an entry.
	if (errRc != ESP_OK) {
		ESP_LOGE("esp_ble_gap_update_whitelist: rc=%d %s", errRc, GeneralUtils::errorToString(errRc));
	}
	ESP_LOGV("<< whiteListAdd");
} // whiteListAdd
...
void BLEDevice::whiteListRemove(BLEAddress address) {
	ESP_LOGV(">> whiteListRemove: %s", address.toString().c_str());
	esp_err_t errRc = esp_ble_gap_update_whitelist(false, *address.getNative());  // False to remove an entry.
	if (errRc != ESP_OK) {
		ESP_LOGE("esp_ble_gap_update_whitelist: rc=%d %s", errRc, GeneralUtils::errorToString(errRc));
	}
	ESP_LOGV("<< whiteListRemove");
} // whiteListRemove
...
esp_err_t BLEDevice::setMTU(uint16_t mtu) {
	ESP_LOGV(">> setLocalMTU: %d", mtu);
	esp_err_t err = esp_ble_gatt_set_local_mtu(mtu);
	if (err == ESP_OK) {
		m_localMTU = mtu;
	} else {
		ESP_LOGE("can't set local mtu value: %d", mtu);
	}
	ESP_LOGV("<< setLocalMTU");
	return err;
}
...
BLEAdvertising* BLEDevice::getAdvertising() {
	if(m_bleAdvertising == nullptr) {
		...
		ESP_LOGI("create advertising");
	}
	ESP_LOGD("get advertising");
...
void BLEDevice::updatePeerDevice(void* peer, bool _client, uint16_t conn_id) {
	ESP_LOGD("update conn_id: %d, GATT role: %s", conn_id, _client? "client":"server");
...
void BLEDevice::addPeerDevice(void* peer, bool _client, uint16_t conn_id) {
	ESP_LOGI("add conn_id: %d, GATT role: %s", conn_id, _client? "client":"server");
...
void BLEDevice::removePeerDevice(uint16_t conn_id, bool _client) {
	ESP_LOGI("remove: %d, GATT role %s", conn_id, _client?"client":"server");
