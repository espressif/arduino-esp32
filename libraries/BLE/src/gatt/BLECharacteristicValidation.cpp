...
#include "gatt/BLECharacteristicValidation.h"
#include "core/BLEBackend.h"  // full BLECharacteristic::Impl / BLEDescriptor::Impl
...
// SIG-assigned 16-bit UUIDs for GATT-defined descriptors (aliases of the shared
// definitions in BLEProperty.h). BT Core Spec v5.x, Vol 3, Part G, §3.3.3.
constexpr uint16_t DSC_UUID_EXT_PROPS = BLE_DSC_UUID16_EXT_PROPS;  // §3.3.3.1
constexpr uint16_t DSC_UUID_USER_DESC = BLE_DSC_UUID16_USER_DESC;  // §3.3.3.2
constexpr uint16_t DSC_UUID_CCCD = BLE_DSC_UUID16_CCCD;            // §3.3.3.3
constexpr uint16_t DSC_UUID_SCCD = BLE_DSC_UUID16_SCCD;            // §3.3.3.4
constexpr uint16_t DSC_UUID_PRES_FMT = BLE_DSC_UUID16_PRES_FMT;    // §3.3.3.5
 ...
 */
const BLEDescriptor::Impl *findDescriptorByUuid16(const BLECharacteristic::Impl &chr, uint16_t uuid16) {
  BLEUUID want(uuid16);
...
// Contract documented on the declaration in BLECharacteristicValidation.h.
bool bleValidateCharProps(const BLEUUID &uuid, BLEProperty props, BLEPermission perms) {
  ...
  // most stacks fall back to (or reject) the write. BT Core Spec v5.x, Vol 3,
  // Part G, §3.3.1.1 and Part H, §2.4.2.4.
  if ((props & BLEProperty::SignedWrite) && !((perms & BLEPermission::WriteEncrypted) || (perms & BLEPermission::WriteAuthenticated))) {
    log_w("Characteristic %s: SIGNED_WRITE without an encrypted/authenticated write permission — signed writes require a bonded link", u);
  }
...
// Contract documented on the declaration in BLECharacteristicValidation.h.
bool bleValidateCharForRegistration(const BLECharacteristic::Impl &chr, bool stackIsNimble) {
  ...
  const BLEDescriptor::Impl *cccd = findDescriptorByUuid16(chr, DSC_UUID_CCCD);
  const BLEDescriptor::Impl *sccd = findDescriptorByUuid16(chr, DSC_UUID_SCCD);
  const BLEDescriptor::Impl *extProp = findDescriptorByUuid16(chr, DSC_UUID_EXT_PROPS);
  const BLEDescriptor::Impl *presFmt = findDescriptorByUuid16(chr, DSC_UUID_PRES_FMT);
  ...
  // The reverse also holds (§3.3.3.1): the Extended Properties descriptor is
  // present if and only if the Extended Properties bit is set.
  if (extProp && !(props & BLEProperty::ExtendedProps)) {
    log_e("Characteristic %s: descriptor 0x2900 present but EXTENDED_PROPERTIES property not set", u);
    ok = false;
  }

  // 2.8.1 User Description (0x2901) writability is gated by the Extended
  // Properties "Writable Auxiliaries" flag (bit 1 of the 0x2900 value,
  // §3.3.3.1 / §3.3.3.2). A writable 0x2901 without that bit set is a
  // misconfiguration most peers will reject on write.
  const BLEDescriptor::Impl *userDesc = findDescriptorByUuid16(chr, DSC_UUID_USER_DESC);
  if (userDesc && hasAnyWritePerm(userDesc->permissions)) {
    const bool auxWritable = extProp && !extProp->value.empty() && (extProp->value[0] & 0x02);
    if (!auxWritable) {
      log_w(
        "Characteristic %s: User Description (0x2901) is writable but the Extended "
        "Properties Writable Auxiliaries bit (0x2900 bit 1) is not set",
        u
      );
    }
  }
