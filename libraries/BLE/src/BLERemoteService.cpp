...
void BLERemoteService::gattClientEventHandler(
	...
	/*
		case ESP_GATTC_GET_CHAR_EVT: {
			...
			if (errRc != ESP_OK) {
				ESP_LOGE("esp_ble_gattc_get_characteristic: rc=%d %s", errRc, GeneralUtils::errorToString(errRc));
				break;
			}
...
void BLERemoteService::retrieveCharacteristics() {
	ESP_LOGV(">> getCharacteristics() for service: %s", getUUID().toString().c_str());
	...
	while (true) {
		...
		if (status != ESP_GATT_OK) {   // If we got an error, end.
			ESP_LOGE("esp_ble_gattc_get_all_char: %s", BLEUtils::gattStatusToString(status).c_str());
			break;
		}
		...
		ESP_LOGD("Found a characteristic: Handle: %d, UUID: %s", result.char_handle, BLEUUID(result.uuid).toString().c_str());
...
uint16_t BLERemoteService::getHandle() {
	ESP_LOGV(">> getHandle: service: %s", getUUID().toString().c_str());
	ESP_LOGV("<< getHandle: %d 0x%.2x", getStartHandle(), getStartHandle());
...
std::string BLERemoteService::getValue(BLEUUID characteristicUuid) {
	ESP_LOGV(">> readValue: uuid: %s", characteristicUuid.toString().c_str());
...
void BLERemoteService::setValue(BLEUUID characteristicUuid, std::string value) {
	ESP_LOGV(">> setValue: uuid: %s", characteristicUuid.toString().c_str());
