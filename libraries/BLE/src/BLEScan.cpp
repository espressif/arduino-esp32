		...
		case ESP_GAP_BLE_SCAN_RESULT_EVT: {
			...
			switch(param->scan_rst.search_evt) {
				...
				case ESP_GAP_SEARCH_INQ_CMPL_EVT: {
					ESP_LOGW("ESP_GAP_SEARCH_INQ_CMPL_EVT");
					m_stopped = true;
...
// ignore it.
					...
					if (found && !m_wantDuplicates) {  // If we found a previous entry AND we don't want duplicates, then we are done.
						ESP_LOGD("Ignoring %s, already seen it.", advertisedAddress.toString().c_str());
					...
					// ESP_LOGW("bytes length: %d + %d, addr type: %d", param->scan_rst.adv_data_len, param->scan_rst.scan_rsp_len, param->scan_rst.ble_addr_type);
...
bool BLEScan::start(uint32_t duration, void (*scanCompleteCB)(BLEScanResults), bool is_continue) {
	ESP_LOGV(">> start(duration=%d)", duration);
	...
	if (errRc != ESP_OK) {
		ESP_LOGE("esp_ble_gap_set_scan_params: err: %d, text: %s", errRc, GeneralUtils::errorToString(errRc));
		m_semaphoreScanEnd.give();
		return false;
	}
	...
	if (errRc != ESP_OK) {
		ESP_LOGE("esp_ble_gap_start_scanning: err: %d, text: %s", errRc, GeneralUtils::errorToString(errRc));
		m_semaphoreScanEnd.give();
		return false;
	}
...
void BLEScan::stop() {
	...
	if (errRc != ESP_OK) {
		ESP_LOGE("esp_ble_gap_stop_scanning: err: %d, text: %s", errRc, GeneralUtils::errorToString(errRc));
		return;
	}
...
void BLEScan::erase(BLEAddress address) {
	ESP_LOGI("erase device: %s", address.toString().c_str());
...
void BLEScanResults::dump() {
	ESP_LOGV(">> Dump scan results:");
	for (int i=0; i<getCount(); i++) {
		ESP_LOGD("- %s", getDevice(i).toString().c_str());
	}
} // dump
