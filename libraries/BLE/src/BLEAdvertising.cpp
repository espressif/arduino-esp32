...
void BLEAdvertising::setScanFilter(bool scanRequestWhitelistOnly, bool connectWhitelistOnly) {
	ESP_LOGV(">> setScanFilter: scanRequestWhitelistOnly: %d, connectWhitelistOnly: %d", scanRequestWhitelistOnly, connectWhitelistOnly);
	if (!scanRequestWhitelistOnly && !connectWhitelistOnly) {
		m_advParams.adv_filter_policy = ADV_FILTER_ALLOW_SCAN_ANY_CON_ANY;
		ESP_LOGV("<< setScanFilter");
		return;
	}
	if (scanRequestWhitelistOnly && !connectWhitelistOnly) {
...
void BLEAdvertising::setAdvertisementData(BLEAdvertisementData& advertisementData) {
	ESP_LOGV(">> setAdvertisementData");
	esp_err_t errRc = ::esp_ble_gap_config_adv_data_raw(
		(uint8_t*)advertisementData.getPayload().data(),
		advertisementData.getPayload().length());
	if (errRc != ESP_OK) {
		ESP_LOGE("esp_ble_gap_config_adv_data_raw: %d %s", errRc, GeneralUtils::errorToString(errRc));
	}
...
void BLEAdvertising::setScanResponseData(BLEAdvertisementData& advertisementData) {
	ESP_LOGV(">> setScanResponseData");
	esp_err_t errRc = ::esp_ble_gap_config_scan_rsp_data_raw(
		(uint8_t*)advertisementData.getPayload().data(),
		advertisementData.getPayload().length());
	if (errRc != ESP_OK) {
		ESP_LOGE("esp_ble_gap_config_scan_rsp_data_raw: %d %s", errRc, GeneralUtils::errorToString(errRc));
	}
...
void BLEAdvertising::start() {
	ESP_LOGV(">> start: customAdvData: %d, customScanResponseData: %d", m_customAdvData, m_customScanResponseData);
	...
	if (numServices > 0) {
		...
		for (int i = 0; i < numServices; i++) {
			ESP_LOGD("- advertising service: %s", m_serviceUUIDs[i].toString().c_str());
		...
		m_advData.service_uuid_len = 0;
		ESP_LOGD("- no services advertised");
	}
	...
	if (!m_customAdvData) {
		...
		if (errRc != ESP_OK) {
			ESP_LOGE("<< esp_ble_gap_config_adv_data: rc=%d %s", errRc, GeneralUtils::errorToString(errRc));
			return;
		}
	}
	...
	if (!m_customScanResponseData && m_scanResp) {
		...
		errRc = ::esp_ble_gap_config_adv_data(&m_advData);
		if (errRc != ESP_OK) {
			ESP_LOGE("<< esp_ble_gap_config_adv_data (Scan response): rc=%d %s", errRc, GeneralUtils::errorToString(errRc));
			return;
		}
	}
	...
	if (errRc != ESP_OK) {
		ESP_LOGE("<< esp_ble_gap_start_advertising: rc=%d %s", errRc, GeneralUtils::errorToString(errRc));
		return;
	}
	ESP_LOGV("<< start");
} // start
...
void BLEAdvertising::stop() {
	...
	if (errRc != ESP_OK) {
		ESP_LOGE("esp_ble_gap_stop_advertising: rc=%d %s", errRc, GeneralUtils::errorToString(errRc));
		return;
	}
	ESP_LOGV("<< stop");
} // stop
...
void BLEAdvertising::handleGAPEvent(
	...
	ESP_LOGD("handleGAPEvent [event no: %d]", (int)event);
