...
void BLEService::executeCreate(BLEServer* pServer) {
	ESP_LOGV(">> executeCreate() - Creating service (esp_ble_gatts_create_service) service uuid: %s", getUUID().toString().c_str());
	m_pServer          = pServer;
	...
	if (errRc != ESP_OK) {
		ESP_LOGE("esp_ble_gatts_create_service: rc=%d %s", errRc, GeneralUtils::errorToString(errRc));
		return;
	}
...
void BLEService::executeDelete() {
	...
	if (errRc != ESP_OK) {
		ESP_LOGE("esp_ble_gatts_delete_service: rc=%d %s", errRc, GeneralUtils::errorToString(errRc));
		return;
	}
...
void BLEService::dump() {
	ESP_LOGD("Service: uuid:%s, handle: 0x%.2x",
		m_uuid.toString().c_str(),
		m_handle);
	ESP_LOGD("Characteristics:\n%s", m_characteristicMap.toString().c_str());
} // dump
...
//
	ESP_LOGV(">> start(): Starting service (esp_ble_gatts_start_service): %s", toString().c_str());
	if (m_handle == NULL_HANDLE) {
		ESP_LOGE("<< !!! We attempted to start a service but don't know its handle!");
		return;
	}
	...
	if (errRc != ESP_OK) {
		ESP_LOGE("<< esp_ble_gatts_start_service: rc=%d %s", errRc, GeneralUtils::errorToString(errRc));
		return;
	}
...
// We start the service through its local handle which was returned in the ESP_GATTS_CREATE_EVT event
// obtained as a result of calling esp_ble_gatts_create_service().
	ESP_LOGV(">> stop(): Stopping service (esp_ble_gatts_stop_service): %s", toString().c_str());
	if (m_handle == NULL_HANDLE) {
		ESP_LOGE("<< !!! We attempted to stop a service but don't know its handle!");
		return;
	}
	...
	if (errRc != ESP_OK) {
		ESP_LOGE("<< esp_ble_gatts_stop_service: rc=%d %s", errRc, GeneralUtils::errorToString(errRc));
		return;
	}
...
void BLEService::setHandle(uint16_t handle) {
	ESP_LOGV(">> setHandle - Handle=0x%.2x, service UUID=%s)", handle, getUUID().toString().c_str());
	if (m_handle != NULL_HANDLE) {
		ESP_LOGE("!!! Handle is already set %.2x", m_handle);
		return;
	}
	m_handle = handle;
	ESP_LOGV("<< setHandle");
...
void BLEService::addCharacteristic(BLECharacteristic* pCharacteristic) {
	...
	ESP_LOGV(">> addCharacteristic()");
	ESP_LOGD("Adding characteristic: uuid=%s to service: %s",
		pCharacteristic->getUUID().toString().c_str(),
		toString().c_str());

	// Check that we don't add the same characteristic twice.
	if (m_characteristicMap.getByUUID(pCharacteristic->getUUID()) != nullptr) {
		ESP_LOGW("<< Adding a new characteristic with the same UUID as a previous one");
		//return;
	}
		...
		case ESP_GATTS_ADD_CHAR_EVT: {
			if (m_handle == param->add_char.service_handle) {
				...
				if (pCharacteristic == nullptr) {
					ESP_LOGE("Expected to find characteristic with UUID: %s, but didnt!",
