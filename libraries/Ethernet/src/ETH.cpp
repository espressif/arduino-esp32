...
bool ETHClass::begin(eth_phy_type_t type, uint8_t phy_addr, int mdc, int mdio, int power, eth_clock_mode_t clock_mode)
{
    ...
    if(!perimanDetachPin(_pin_rmii_clock)){ return false; }
    if(!perimanDetachPin(_pin_mcd)){ return false; }
    if(!perimanDetachPin(_pin_mdio)){ return false; }
    if(!perimanDetachPin(ETH_RMII_TX_EN)){ return false; }
    if(!perimanDetachPin(ETH_RMII_TX0)){ return false; }
    if(!perimanDetachPin(ETH_RMII_TX1)){ return false; }
    if(!perimanDetachPin(ETH_RMII_RX0)){ return false; }
    if(!perimanDetachPin(ETH_RMII_RX1_EN)){ return false; }
    if(!perimanDetachPin(ETH_RMII_CRS_DV)){ return false; }
    if(_pin_power != -1){
        if(!perimanDetachPin(_pin_power)){ return false; }
    }
    ...
    if(!perimanSetPinBus(_pin_rmii_clock, ESP32_BUS_TYPE_ETHERNET_CLK, (void *)(this))){ goto err; }
    if(!perimanSetPinBus(_pin_mcd, ESP32_BUS_TYPE_ETHERNET_MCD, (void *)(this))){ goto err; }
    if(!perimanSetPinBus(_pin_mdio,  ESP32_BUS_TYPE_ETHERNET_MDIO, (void *)(this))){ goto err; }

    if(!perimanSetPinBus(ETH_RMII_TX_EN, ESP32_BUS_TYPE_ETHERNET_RMII, (void *)(this))){ goto err; }
    if(!perimanSetPinBus(ETH_RMII_TX0, ESP32_BUS_TYPE_ETHERNET_RMII, (void *)(this))){ goto err; }
    if(!perimanSetPinBus(ETH_RMII_TX1,  ESP32_BUS_TYPE_ETHERNET_RMII, (void *)(this))){ goto err; }
    if(!perimanSetPinBus(ETH_RMII_RX0, ESP32_BUS_TYPE_ETHERNET_RMII, (void *)(this))){ goto err; }
    if(!perimanSetPinBus(ETH_RMII_RX1_EN, ESP32_BUS_TYPE_ETHERNET_RMII, (void *)(this))){ goto err; }
    if(!perimanSetPinBus(ETH_RMII_CRS_DV,  ESP32_BUS_TYPE_ETHERNET_RMII, (void *)(this))){ goto err; }

    if(_pin_power != -1){
        if(!perimanSetPinBus(_pin_power,  ESP32_BUS_TYPE_ETHERNET_PWR, (void *)(this))){ goto err; }
    }
...
bool ETHClass::beginSPI(eth_phy_type_t type, uint8_t phy_addr, int cs, int irq, int rst, 
    ...
    if(_pin_cs != -1){
        if(!perimanDetachPin(_pin_cs)){ return false; }
    }
    if(_pin_rst != -1){
        if(!perimanDetachPin(_pin_rst)){ return false; }
    }
    if(_pin_irq != -1){
        if(!perimanDetachPin(_pin_irq)){ return false; }
    }
    if(_pin_sck != -1){
        if(!perimanDetachPin(_pin_sck)){ return false; }
    }
    if(_pin_miso != -1){
        if(!perimanDetachPin(_pin_miso)){ return false; }
    }
    if(_pin_mosi != -1){
        if(!perimanDetachPin(_pin_mosi)){ return false; }
    }
    ...
    if(_spi == NULL){
#endif
        if(!perimanSetPinBus(_pin_cs, ESP32_BUS_TYPE_ETHERNET_SPI, (void *)(this))){ goto err; }
#if ETH_SPI_SUPPORTS_CUSTOM
    }
#endif
    if(!perimanSetPinBus(_pin_irq, ESP32_BUS_TYPE_ETHERNET_SPI, (void *)(this))){ goto err; }

    if(_pin_sck != -1){
        if(!perimanSetPinBus(_pin_sck, ESP32_BUS_TYPE_ETHERNET_SPI, (void *)(this))){ goto err; }
    }
    if(_pin_miso != -1){
        if(!perimanSetPinBus(_pin_miso, ESP32_BUS_TYPE_ETHERNET_SPI, (void *)(this))){ goto err; }
    }
    if(_pin_mosi != -1){
        if(!perimanSetPinBus(_pin_mosi,  ESP32_BUS_TYPE_ETHERNET_SPI, (void *)(this))){ goto err; }
    }
    if(_pin_rst != -1){
        if(!perimanSetPinBus(_pin_rst,  ESP32_BUS_TYPE_ETHERNET_SPI, (void *)(this))){ goto err; }
    }
...
void ETHClass::end(void)
{
    ...
    if(_pin_rmii_clock != -1 && _pin_mcd != -1 && _pin_mdio != -1){
        perimanDetachPin(_pin_rmii_clock);
        perimanDetachPin(_pin_mcd);
        perimanDetachPin(_pin_mdio);

        perimanDetachPin(ETH_RMII_TX_EN);
        perimanDetachPin(ETH_RMII_TX0);
        perimanDetachPin(ETH_RMII_TX1);
        perimanDetachPin(ETH_RMII_RX0);
        perimanDetachPin(ETH_RMII_RX1_EN);
        perimanDetachPin(ETH_RMII_CRS_DV);
    ...
    if(_pin_power != -1){
        perimanDetachPin(_pin_power);
        _pin_power = -1;
    }
#endif /* CONFIG_ETH_USE_ESP32_EMAC */
    if(_pin_cs != -1){
        perimanDetachPin(_pin_cs);
        _pin_cs = -1;
    }
    if(_pin_irq != -1){
        perimanDetachPin(_pin_irq);
        _pin_irq = -1;
    }
    if(_pin_sck != -1){
        perimanDetachPin(_pin_sck);
        _pin_sck = -1;
    }
    if(_pin_miso != -1){
        perimanDetachPin(_pin_miso);
        _pin_miso = -1;
    }
    if(_pin_mosi != -1){
        perimanDetachPin(_pin_mosi);
        _pin_mosi = -1;
    }
    if(_pin_rst != -1){
        perimanDetachPin(_pin_rst);
        _pin_rst = -1;
    }
}
