...
#define AUDIO_MODE                                                             \
  AUDIO_MODE_LOOPBACK // <-- change to AUDIO_MODE_TONE to test TX only
...
void generateToneSamples(int16_t *buf, uint16_t sample_count,
...
{
  Serial.printf("%02X:%02X:%02X:%02X:%02X:%02X", bda[0], bda[1], bda[2], bda[3],
                bda[4], bda[5]);
}
...
void tryConnectAudio(const char *reason)
{
  ...
  const uint32_t now_ms = millis();
  if (audio_connecting &&
      (now_ms - last_audio_connect_attempt_ms) < kAudioConnectRetryMs) // already attempting; wait before retrying
    return;
...
void connectFirstBondedDevice()
{
  ...
  {
    Serial.println(
        "No bonded phone yet. Pair from the phone's Bluetooth menu.");
    return;
  }
  ...
  esp_err_t err =
      esp_bt_gap_get_bond_device_list(&device_count, bonded_devices); // retrieve the actual addresses
...
void gapCallback(esp_bt_gap_cb_event_t event, esp_bt_gap_cb_param_t *param)
{
  ...
  case ESP_BT_GAP_CFM_REQ_EVT:
    ...
    logEspCall("esp_bt_gap_ssp_confirm_reply",
               esp_bt_gap_ssp_confirm_reply(param->cfm_req.bda, true)); // auto-accept: skip user confirmation for this example
    break;
  ...
  case ESP_BT_GAP_PIN_REQ_EVT:
  {
    ...
    logEspCall("esp_bt_gap_pin_reply",
               esp_bt_gap_pin_reply(param->pin_req.bda, true, 4, pin_code)); // respond with PIN "0000"
    break;
  }
                     ...
                     esp_hf_client_cb_param_t *param)
{
  ...
  case ESP_HF_CLIENT_CONNECTION_STATE_EVT:
    ...
    Serial.printf("HFP connection state: %s peer_feat=0x%08lX chld=0x%08lX ",
                  hfConnStateName(param->conn_stat.state),
    ...
    if (slc_connected)
    {
      ...
      logEspCall(
          "esp_hf_client_volume_update(mic)",
...
void setup()
{
  ...
  Serial.println();
  Serial.printf("ESP32 HFP HCI Audio example — mode: %s\n",
#if AUDIO_MODE == AUDIO_MODE_TONE
  ...
  logEspCall("esp_bt_dev_set_device_name",
             esp_bt_dev_set_device_name("ESP32_HFP_AUDIO"));
  ...
  logEspCall(
      ...
      esp_hf_client_register_data_callback(hfIncomingDataCb, hfOutgoingDataCb));
  ...
  if (audio_connected)
  {
    ...
    if (now_ms - last_audio_debug_ms >= 1000) // print diagnostics once per second
    {
      ...
      Serial.printf(
          ...
          is_msbc ? "mSBC/16kHz" : "CVSD/8kHz",
          static_cast<unsigned long>(cb_count),
          static_cast<unsigned long>(cb_bytes),
          static_cast<unsigned long>(cb_last_len),
          static_cast<unsigned long>(cb_underrun));
