...
//
// Routes SCO audio between a paired phone (HFP over HCI) and external
// audio hardware:
...
// requires a system clock (SCKI); this is produced by the APLL and routed
// to a GPIO via the ESP32 CLK_OUT multiplexer (only GPIO 0, 1, or 3
// support this).
//
...
#include "driver/i2s.h"                 // I2S DMA driver
...
#define PIN_I2S_SCKI  0  // Master clock   — APLL output → PCM1808 SCKI.
                         // On the ESP32, I2S0 MCLK is hardware-tied to GPIO0
                         // (CLK_OUT1).  GPIO1 = UART0 TX and GPIO3 = UART0 RX;
                         // using either would reconfigure the UART pin and kill
                         // Serial output.  GPIO0 is the only valid choice for
                         // I2S_NUM_0.

#define PIN_ANSWER   16  // Answer button — active-low with internal pull-up.
...
// PCM1808 SCKI = kMclkMult × fs.  512× works for both codecs:
//   CVSD  8 kHz → 4.096 MHz,  mSBC 16 kHz → 8.192 MHz
constexpr uint32_t kMclkMult = 512;

// I2S DMA buffer sizing.
constexpr int    kI2sDmaBufLen   = 64;  // stereo frames per DMA buffer
constexpr int    kI2sDmaBufCount = 6;   // number of DMA buffers
constexpr size_t kAdcRingFrames  = 3;   // SCO frames buffered for jitter absorption
...
void startI2s(bool msbc)
{
  ...
  const uint32_t mclk = kMclkMult * fs;

  const i2s_config_t cfg = {
    // Master mode with simultaneous TX (playback) and RX (capture).
    .mode                 = static_cast<i2s_mode_t>(I2S_MODE_MASTER |
                                                    I2S_MODE_TX     |
                                                    I2S_MODE_RX),
    .sample_rate          = fs,
    // 32-bit slots give BCK = 64×fs, which the PCM1808 requires.
    // With 16-bit slots BCK = 32×fs and the PCM1808 cannot clock out its
    // 24-bit data, resulting in silence on the ADC path.
    .bits_per_sample      = I2S_BITS_PER_SAMPLE_32BIT,
    // Stereo layout: the I2S bus always clocks two channels (L + R) per frame,
    // even though HFP SCO carries mono.  Conversion is done in the callbacks.
    .channel_format       = I2S_CHANNEL_FMT_RIGHT_LEFT,
    // Standard I2S framing: LRCK transitions one BCK before the MSB,
    // data is MSB-first, and the word length equals bits_per_sample.
    .communication_format = I2S_COMM_FORMAT_STAND_I2S,
    .intr_alloc_flags     = ESP_INTR_FLAG_LEVEL1,
    .dma_buf_count        = kI2sDmaBufCount,
    .dma_buf_len          = kI2sDmaBufLen,
    // APLL gives a low-jitter, accurate sample clock.  Without it the RC
    // oscillator can drift enough to cause audible pitch shift at 8 kHz.
    .use_apll             = true,
    // If the TX DMA queue is not fed in time, output silence automatically
    // rather than repeating stale data.
    .tx_desc_auto_clear   = true,
    // Request APLL to lock to this MCLK frequency; the driver tunes it and
    // then derives BCK and LRCK via integer dividers.
    .fixed_mclk           = static_cast<int>(mclk),
  };

  const i2s_pin_config_t pins = {
    .mck_io_num   = PIN_I2S_SCKI, // APLL MCLK output → PCM1808 SCKI
    .bck_io_num   = PIN_I2S_BCK,  // bit clock shared with both codecs
    .ws_io_num    = PIN_I2S_LRCK, // word select shared with both codecs
    .data_out_num = PIN_I2S_DOUT, // ESP32 → PCM5102A (DAC input)
    .data_in_num  = PIN_I2S_DIN,  // PCM1808 (ADC output) → ESP32
  };

  logEspCall("i2s_driver_install",
             i2s_driver_install(kI2sPort, &cfg, 0, nullptr));
  logEspCall("i2s_set_pin",
             i2s_set_pin(kI2sPort, &pins));

  i2s_running    = true;
  Serial.printf("I2S started: %s  fs=%" PRIu32 " Hz  SCKI=%" PRIu32 " Hz\n",
                msbc ? "mSBC" : "CVSD", fs, mclk);
}

// Uninstall the I2S driver.  Flag is cleared first so the ADC task stops
// issuing reads; a 20 ms delay lets any in-flight read finish before teardown.
void stopI2s()
{
  if (!i2s_running) return;
  // Clear the flag BEFORE uninstalling so that the ADC capture task stops
  // issuing new i2s_read calls immediately.  A brief delay follows to let any
  // read already in progress finish before the driver is removed.
  i2s_running = false;
  vTaskDelay(pdMS_TO_TICKS(20));
  // Clear any queued playback samples so disconnect does not end with a burst
  // of stale DAC data from the TX DMA ring.
  logEspCall("i2s_zero_dma_buffer", i2s_zero_dma_buffer(kI2sPort));
  logEspCall("i2s_driver_uninstall", i2s_driver_uninstall(kI2sPort));
...
void adcCaptureTask(void *)
{
  ...
  uint8_t *rx_bytes = reinterpret_cast<uint8_t *>(rx_stereo);

  while (adc_task_running)
  {
    size_t bytes_read = 0;
    while (adc_task_running && bytes_read < stereo_bytes)
    {
      size_t chunk_read = 0;
      const esp_err_t err = i2s_read(kI2sPort,
                                     rx_bytes + bytes_read,
                                     stereo_bytes - bytes_read,
                                     &chunk_read,
                                     pdMS_TO_TICKS(20));
    ...
    for (size_t i = 0; i < n_mono; ++i)
    {
      ...
      const int16_t p = static_cast<int16_t>(abs(static_cast<int32_t>(s)));
      if (p > rx_peak_adc) rx_peak_adc = p;
    }
...
void hfIncomingDataCb(const uint8_t *buf, uint32_t len)
{
  ...
  // Write the stereo frame to the I2S TX DMA ring buffer.
  size_t written = 0;
  i2s_write(kI2sPort, tx_stereo, n_mono * 2 * sizeof(int32_t), &written,
            pdMS_TO_TICKS(10));
}
...
void setup()
{
  ...
  pinMode(PIN_ANSWER, INPUT_PULLUP);
...
void loop()
{
  ...
  // Answer button: detect falling edge on PIN_ANSWER (HIGH → LOW).
  // Answer the call only on the press edge so holding the button does not
  // repeat the API call.
  static bool btn_prev = HIGH;
  const bool  btn_now  = digitalRead(PIN_ANSWER);
