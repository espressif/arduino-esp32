...
static void bind_cb(esp_zb_zdp_status_t zdo_status, void *user_ctx) {
  ...
  if (zdo_status == ESP_ZB_ZDP_STATUS_SUCCESS) {
      ...
      if (bind_req->req_dst_addr == esp_zb_get_short_address()) {
          log_i("Successfully bind the temperature sensor from address(0x%x) on endpoint(%d)",
      ...
      if (bind_req->req_dst_addr == temp_sensor.short_addr) {
          log_i("The temperature sensor from address(0x%x) on endpoint(%d) successfully binds us",
                    temp_sensor.short_addr, temp_sensor.endpoint);
      }
      free(bind_req);
  }
...
static esp_err_t zb_attribute_reporting_handler(const esp_zb_zcl_report_attr_message_t *message)
{
  ...
  log_i("Received report from address(0x%x) src endpoint(%d) to dst endpoint(%d) cluster(0x%x)",
...
static esp_err_t zb_read_attr_resp_handler(const esp_zb_zcl_cmd_read_attr_resp_message_t *message)
{
  ...
  log_i("Read attribute response: from address(0x%x) src endpoint(%d) to dst endpoint(%d) cluster(0x%x)",
            message->info.src_address.u.short_addr, message->info.src_endpoint,
            message->info.dst_endpoint, message->info.cluster);

  esp_zb_zcl_read_attr_resp_variable_t *variable = message->variables;
  while (variable) {
      log_i("Read attribute response: status(%d), cluster(0x%x), attribute(0x%x), type(0x%x), value(%d)",
...
static esp_err_t zb_configure_report_resp_handler(const esp_zb_zcl_cmd_config_report_resp_message_t *message)
{
  ...
  while (variable) {
      log_i("Configure report response: status(%d), cluster(0x%x), direction(0x%x), attribute(0x%x)",
