	...
	do {
		...
		*/		
		if(rootCABuff != NULL) {
			...
			ret = mbedtls_x509_crt_parse(&ssl_client->ca_cert, rootCABuff, strlen((const char *)rootCABuff)+1);
		...
		if(cli_cert != NULL && cli_key != NULL) {
			...
			ret = mbedtls_x509_crt_parse(&ssl_client->client_cert, cli_cert, strlen((const char *)cli_cert)+1);
			...
			ret = mbedtls_pk_parse_key(&ssl_client->client_key, cli_key, strlen((const char *)cli_key)+1, NULL, 0); 
...
/*    uint8_t has_backup_recvtimeout = 0;
	...
	printf("Length: %i \n",length);
    if (ssl_client->ssl_ctx == NULL) {
        return 0;
    }

    if (flag & 0x01) {
        // peek for 10ms
        ret = lwip_getsockopt(ssl_client->fd, SOL_SOCKET, SO_RCVTIMEO, &backup_recv_timeout, &len);
        if (ret >= 0) {
            recv_timeout = 10;
            ret = lwip_setsockopt(ssl_client->fd, SOL_SOCKET, SO_RCVTIMEO, &recv_timeout, sizeof(recv_timeout));
            if (ret >= 0) {
...
/*
    if ((flag & 0x01) && (has_backup_recvtimeout == 1)) {
        ...
        lwip_setsockopt(ssl_client->fd, SOL_SOCKET, SO_RCVTIMEO, &backup_recv_timeout, sizeof(recv_timeout));
    }
*/	
