...
{
    int ret = start_ssl_client(sslclient, ip, port, rootCABuff, rootCA_len, cli_cert, cli_cert_len, cli_key, cli_key_len);
    if (ret < 0) {
...
int WiFiClientSecure::connect(const char *host, uint16_t port, const uint8_t *rootCABuff, size_t rootCA_len, const uint8_t *cli_cert, size_t cli_cert_len, const uint8_t *cli_key, size_t cli_key_len)
{
    ...
    return connect(srv, port, rootCABuff, rootCA_len, cli_cert, cli_cert_len, cli_key, cli_key_len);
}
