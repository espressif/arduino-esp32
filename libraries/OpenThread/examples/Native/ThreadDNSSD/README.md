...
- **Fatal setup:** `FAIL: not attached` / `FAIL: OThreadDNSSD.begin` (and similar) **halt** simple sketches (`while (true) { delay(1000); }`). Returning from `setup()` still runs `loop()`, which would look like a live browse/announce with no client. `waitForAnnounce()` timeout is **not** fatal — Advertise keeps polling `isAnnounceComplete()`. `ThreadDNSSD_Advertise_Callback` and the UDP Light re-advertise from `loop()` after OTBR restart / lost attach (light does not halt on a failed recovery `begin()`).
